...
def detect_query_clauses(user_input):
    pattern = (
        r"(?:select (?P<select>[\w, ]+))?"
        r"(?: where (?P<where>.+?))?"
        r"(?: group by (?P<group_by>[\w, ]+))?"
        r"(?: having (?P<having>.+?))?"
        r"(?: order by (?P<order_by>[\w, ]+))?"
    )
    ...
    return {key: value.strip() for key, value in clauses.items() if value}
...
def handle_natural_language_query(db_type, table_name, user_input):
    ...
    # clauses = detect_query_clauses(user_input)
    # print(clauses)
    # if not clauses:
